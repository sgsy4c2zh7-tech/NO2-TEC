...
async function pruneOldArchives(nowUtc = new Date(), keepDays = KEEP_DAYS) {
...
async function main() {
  ...
  try {
    ...
    await pruneOldArchives(new Date(), KEEP_DAYS);
    ...
    console.log(`Retention policy: ${KEEP_DAYS} days`);
  } finally {
    await browser.close();
